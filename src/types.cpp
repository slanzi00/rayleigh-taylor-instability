...
Axis::Axis(double x_max, double y_max)
    : x{Eigen::ArrayXd::LinSpaced(ic::n_x, 0.5 * ic::dx, x_max - 0.5 * ic::dx)}
    , y{Eigen::ArrayXd::LinSpaced(ic::n_y, 0.5 * ic::dx, y_max - 0.5 * ic::dx)}
{
}
...
{
  auto fill_rho = [](uint32_t i, uint32_t j) {
    return (j < ic::n_y / 2) ? 1. : 2.;
  };

  auto perturbation = [&](uint32_t i, uint32_t j) {
    return 0.0025 * (1. - std::cos(4. * M_PI * axis.x(i))) *
           (1. - std::cos(4. / 3. * M_PI * axis.y(j)));
  };

  auto fill_pressure = [&](uint32_t i, uint32_t j) {
    return ic::p_0 + ic::g * (axis.y(j) - 0.75) * rho(j, j);
  };
